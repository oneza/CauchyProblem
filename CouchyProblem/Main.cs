...
/*      double t1 = 0;
      ...
      ITheta theta = thetaloader.FindLib(DLLPath + "CylinderSet_NoConstraints.dll");
      ...
      Grid grid = Grid.BallGrid(center, 4.6, xSteps);
      ControlConstraints P = ControlConstraints.BallConstraints(center, radiusP, pSteps);
      ControlConstraints Q = ControlConstraints.BallConstraints(center, radiusQ, qSteps);*/
      ...
      for (int i = time.Length - 1; i > 0; i--)
      {
        foreach (PhasePoint x in grid.Keys)
        {
          ...
          grid[x].Add(time[i-1], Math.Max(Math.Min(chi1.chi(time[i-1], x), 
            grid[x][time[i]] + (time[i] - time[i-1]) * velocity), 
            theta1.theta(time[i-1], x)));
        }
      }

      using (StreamWriter outputFile = new StreamWriter(Path.Combine(outPath, "Result3.txt")))
      {
        int i = 1;
        foreach (PhasePoint x in grid.Keys)
        {
